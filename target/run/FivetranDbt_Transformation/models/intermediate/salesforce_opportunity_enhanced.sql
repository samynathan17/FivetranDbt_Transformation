  ...
  create or replace  view FIVETRAN_DATABASE.dbt_dataflo_staging.salesforce_opportunity_enhanced  as (
    with opportunity as (
    
    select *
    from FIVETRAN_DATABASE.dbt_dataflo_staging.stg_sf_opportunity

), salesforce_user as (

    select *
    from FIVETRAN_DATABASE.dbt_dataflo_staging.stg_sf_user
    ...
    select 
      opportunity.* ,
      --account.*,
      opportunity_owner.user_id as opportunity_ownerid,
