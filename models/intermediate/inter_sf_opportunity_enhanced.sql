...
WITH account AS (
    ...
    currmonthleadstatus.currentmonthLeads_by_status,
   ...
   --Leads by location -pending
    LEFT JOIN (select lead_owner_id, cast(count(country) as int) as Leadsbylocationlast7days from leads
     WHERE country is not null  and created_at >= DATEADD(day, -7, GETDATE()) 
     group by lead_owner_id) as lastweekleads
    on users.user_id= leads.lead_owner_id

    LEFT JOIN (select leads.lead_owner_id, COUNT(leads.status) as currentmonthLeads_by_status from leads
     WHERE leads.status is not null  and  MONTH(leads.created_at) = MONTH(CURRENT_DATE())
     group by lead_owner_id) as currmonthleadstatus
    on users.user_id= currmonthleadstatus.lead_owner_id
